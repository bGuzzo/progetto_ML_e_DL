...
class EncoderLayer(nn.Module):
    def __init__(self, attention, d_model, d_ff=None, dropout=0.1, activation="relu", l_lstm=0):
        ...
        # First feed forward network as 1D convolution
        ...
        # Define RNN LSTM network as alternative to FeedForward
        self.l_lstm = l_lstm
        if l_lstm != 0:
            # Use RNN LSTM network of the same size of the model
    ...
    def forward(self, x, attn_mask=None):
        ...
        # Output of anomaly attention layer
        x = x + self.dropout(new_x)
        # First normalization layer
        y = x = self.norm1(x)
        # Apply the feed forward network or RNN network
        if self.l_lstm == 0:
            # Apply feed forward
            ...
            y = y_t
        # Sum and second layer normalization
...
class AnomalyTransformer(nn.Module):
    ...
    # Entry point when using the model
